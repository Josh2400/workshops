...
# Advanced 4 - Feature Engieering: dummies and embeddings

## isomap with recipes

The results from isomap unlike UMAP is not able to be distorted in the same way. Here the number of nieghbors change between the charts. Each of the charts are an application of isomap on the same completely random data. The points are in different locations but none of the charts appear to show any patterns.
...
umap_20 <- recipe(~., data_random) |>
  step_isomap(all_predictors(), neighbors = 20, num_terms = 2) |>
  prep() |>
  bake(NULL)

umap_40 <- recipe(~., data_random) |>
  step_isomap(all_predictors(), neighbors = 40, num_terms = 2) |>
  prep() |>
  bake(NULL)
  
umap_all <- bind_rows(
  umap_5 |> mutate(type = "A", neighbors = 5),
  umap_10 |> mutate(type = "B", neighbors = 10),
  umap_20 |> mutate(type = "C", neighbors = 20),
  umap_40 |> mutate(type = "D", neighbors = 40)
)
